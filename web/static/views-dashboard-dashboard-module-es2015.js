...
/* harmony import */ var ngx_bootstrap_popover__WEBPACK_IMPORTED_MODULE_11__ = __webpack_require__(/*! ngx-bootstrap/popover */ "EDFS");
...
/* harmony import */ var ngx_bootstrap_buttons__WEBPACK_IMPORTED_MODULE_22__ = __webpack_require__(/*! ngx-bootstrap/buttons */ "s3T5");
/* harmony import */ var ng2_charts__WEBPACK_IMPORTED_MODULE_23__ = __webpack_require__(/*! ng2-charts */ "LPYB");
/* harmony import */ var _dashboard_component__WEBPACK_IMPORTED_MODULE_24__ = __webpack_require__(/*! ./dashboard.component */ "l70X");
/* harmony import */ var _dashboard_routing_module__WEBPACK_IMPORTED_MODULE_25__ = __webpack_require__(/*! ./dashboard-routing.module */ "jUYC");
...
DashboardModule = Object(tslib__WEBPACK_IMPORTED_MODULE_0__["__decorate"])([
    Object(_angular_core__WEBPACK_IMPORTED_MODULE_3__["NgModule"])({
        imports: [
            ...
            ngx_bootstrap_buttons__WEBPACK_IMPORTED_MODULE_22__["ButtonsModule"].forRoot(),
            highcharts_angular__WEBPACK_IMPORTED_MODULE_26__["HighchartsChartModule"],
            _dashboard_routing_module__WEBPACK_IMPORTED_MODULE_25__["DashboardRoutingModule"],
...
(function(Z,M){ true&&module.exports?(M["default"]=M,module.exports=Z.document?M(Z):M): true?!(__WEBPACK_AMD_DEFINE_RESULT__ = (function(){return M(Z)}).call(exports, __webpack_require__, exports, module),
				__WEBPACK_AMD_DEFINE_RESULT__ !== undefined && (module.exports = __WEBPACK_AMD_DEFINE_RESULT__)):(undefined)})("undefined"!==typeof window?window:this,function(Z){function M(v,a,z,F){v.hasOwnProperty(a)||(v[a]=F.apply(null,z))}var a={};M(a,"Core/Globals.js",[],function(){var v="undefined"!==typeof Z?Z:"undefined"!==typeof window?window:
...
/* harmony import */ var highcharts__WEBPACK_IMPORTED_MODULE_6__ = __webpack_require__(/*! highcharts */ "6n/F");
...
let DashboardComponent = class DashboardComponent {
    constructor(authService, router, _activatedroute, datepipe, loadingController) {
        ...
        this.tmales = [];
        this.tfemales = [];
        this.TotalsObject = [{ "Population_Type": "M", "Value": 0, "gender": "Male" },
            { "Population_Type": "F", "Value": 0, "gender": "Female" },
            { "Population_Type": "Unknown", "Value": 0, "gender": "Unknown" }];
        this.ScreenedTotal = [{ "Population_Type": "M", "Value": 0, "gender": "Male" },
            { "Population_Type": "F", "Value": 0, "gender": "Female" },
            { "Population_Type": "Unknown", "Value": 0, "gender": "Unknown" }];
        this.PopulationObject = [{ "Population_Type": "Staff", "Value": 0 },
            { "Population_Type": "Convict", "Value": 0 },
            { "Population_Type": "Remandee", "Value": 0 }];
        this.newEntrantsObject = [{ "Population_Type": "M", "Value": 0, "gender": "Male" },
            { "Population_Type": "F", "Value": 0, "gender": "Female" },
            { "Population_Type": "Unknown", "Value": 0, "gender": "Unknown" }];
        this.exitedObject = [{ "Population_Type": "M", "Value": 0, "gender": "Male" },
            { "Population_Type": "F", "Value": 0, "gender": "Female" },
            { "Population_Type": "Unknown", "Value": 0, "gender": "Unknown" }];
        this.DataObjects = [
            { "module": "ART", "M": 0, "F": 0, "positive": 0, "negative": 0 },
            { "module": "HTS", "M": 0, "F": 0, "positive": 0, "negative": 0 },
            { "module": "TB", "M": 0, "F": 0, "positive": 0, "negative": 0 },
            { "module": "CXCA", "M": 0, "F": 0, "positive": 0, "negative": 0 },
            { "module": "STI", "M": 0, "F": 0, "positive": 0, "negative": 0 },
            { "module": "COVID", "M": 0, "F": 0, "positive": 0, "negative": 0 }
        ];
        ...
        this.chartOptions = {
            ...
            tooltip: {
                ...
                pointFormat: '<tr><td style="color:{series.color};padding:0">{series.name}: </td>' +
                    '<td style="padding:0"><b>{point.y:f} Inmates</b></td></tr>',
                footerFormat: '</table>',
                shared: true,
                useHTML: true
            },
    ...
    getLoader() {
        this.loadingController.create({ message: "Loading....." }).then(a => {
            a.present().then((b) => Object(tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"])(this, void 0, void 0, function* () {
                let gdata = yield this.getDatasets();
                setTimeout(() => {
                    if (gdata == 1) {
                        a.dismiss();
                        this.loadBidChartData();
    ...
    getPage() {
        this.authService.getPrisoners({ "page_number": this.page_count }).subscribe((data) => {
            this.ItemsCount = data.length;
            this.page_count = this.page_count + 1;
            for (var i = 0; i < data.length; i++) {
                const array = data[i]['person'];
                ...
                this.aged = Math.floor(timeDiff / (1000 * 3600 * 24) / 365.25);
                if (array.dead == 0) {
                    let newentrants = 0;
                    let totalpp = 1;
                    let attributes = array['person_attributes'];
                    var datesub = this.selected_date;
                    let poplntype = this.PopulationObject;
                    Object.keys(attributes).map(function (t) {
                        const attr = attributes[t].type.name;
                        if (attr == "Registration date") { // this is an indicator for number of patients registered
                            const dateTime = attributes[t].value;
                            if (datesub.length != 0) {
                                let fDate = Date.parse(datesub[0]);
                                let lDate = Date.parse(datesub[1]);
                                let cDate = Date.parse(dateTime);
                        ...
                        else if (attr == "Registration Type") { // this is an indicator for number of patients registered
                            let regtype = attributes[t].value;
                            let totalv = poplntype.find(x => x.Population_Type == regtype)["Value"];
                            poplntype.find(x => x.Population_Type == regtype)["Value"] = totalv + 1;
                        }
                    });
                    let totalvr = this.newEntrantsObject.find(x => x.Population_Type == array.gender)["Value"];
                    this.newEntrantsObject.find(x => x.Population_Type == array.gender)["Value"] = totalvr + newentrants;
                    let totalvs = this.TotalsObject.find(x => x.Population_Type == array.gender)["Value"];
                    this.TotalsObject.find(x => x.Population_Type == array.gender)["Value"] = totalvs + 1;
                    var dataobj = { "id": array.person_id,
                        "age": this.aged,
                        "gender": array.gender,
                        "Totalscreened": 0
                    };
                    this.getPatientEncounters(dataobj); // this function gets other encounters of a particular patient          
                }
                else {
                    let totalvx = this.exitedObject.find(x => x.Population_Type == array.gender)["Value"];
                    this.exitedObject.find(x => x.Population_Type == array.gender)["Value"] = totalvx + 1;
                }
            }
    ...
    getPatientEncounters(params) {
        this.authService.getPrisonerEncounters({ "paginate": false, "patient_id": params.id }).subscribe((data) => {
            var artscreen = 0, tbscreen = 0, stiscreen = 0, htsscreen = 0, covidscreen = 0, cxcascreen = 0;
            for (var i = 0; i < data.length; i++) {
                let daterange = false;
                let dateTime = data[i].encounter_datetime.split('T')[0];
                if (this.selected_date.length == 0) {
                    ...
                    let fDate = Date.parse(this.selected_date[0]);
                    let lDate = Date.parse(this.selected_date[1]);
                    let cDate = Date.parse(dateTime);
                    if ((cDate <= lDate && cDate >= fDate)) {
                        daterange = true;
                    }
                ...
                if (daterange == true) {
                    ...
                    if (data[i].encounter_type == 9) {
                        if (artscreen == 0) {
                            var totals = this.DataObjects.find(x => x.module == "ART");
                            this.DataObjects.find(x => x.module == "ART")[gender] = totals[gender] + 1;
                            params.Totalscreened = 1;
                            artscreen = 1;
                            let artno = data[i].observations.find(x => x.concept_id == "7014").value_text;
                            if (artno != null) {
                                this.DataObjects.find(x => x.module == "ART")["positive"] = totals["positive"] + 1;
                            }
                            else {
                                this.DataObjects.find(x => x.module == "ART")["negative"] = totals["negative"] + 1;
                            }
                        }
                    }
                    if (data[i].encounter_type == 78) {
                        if (tbscreen == 0) {
                            var totals = this.DataObjects.find(x => x.module == "TB");
                            this.DataObjects.find(x => x.module == "TB")[gender] = totals[gender] + 1;
                            tbscreen = 1;
                            for (var x = 0; x < data[i].observations.length; x++) {
                                let conceptid = data[i].observations[x].concept_id;
                                if (conceptid == 2136) {
                                    params.Totalscreened = 1;
                                    let tb_results = data[i].observations[x].value_text;
                                    if (tb_results != "presumptive") {
                                        this.DataObjects.find(x => x.module == "TB")[tb_results] = totals[tb_results] + 1;
                                    }
                                }
                            }
                        }
                    ...
                    if (data[i].encounter_type == 146) {
                        if (stiscreen == 0) {
                            for (var x = 0; x < data[i].observations.length; x++) {
                                let conceptid = data[i].observations[x].concept_id;
                                var totals = this.DataObjects.find(x => x.module == "STI");
                                if (conceptid == 1621) {
                                    let screened = data[i].observations[x].value_text;
                                    if (screened == "Yes") {
                                        this.DataObjects.find(x => x.module == "STI")[gender] = totals[gender] + 1;
                                        stiscreen = 1;
                                        params.Totalscreened = 1;
                                        let optn = data[i].observations.find(x => x.concept_id == "2169").value_text;
                                        let sti_results = optn.toLowerCase();
                                        if (sti_results != "") {
                                            this.DataObjects.find(x => x.module == "STI")[sti_results] = totals[sti_results] + 1;
                                        }
                                    }
                                }
                            }
                    ...
                    if (data[i].encounter_type == 163) {
                        if (htsscreen == 0) {
                            for (var x = 0; x < data[i].observations.length; x++) {
                                let conceptid = data[i].observations[x].concept_id;
                                var totals = this.DataObjects.find(x => x.module == "HTS");
                                if (conceptid == 6339) {
                                    let screened = data[i].observations[x].value_text;
                                    if (screened == "Yes") {
                                        this.DataObjects.find(x => x.module == "HTS")[gender] = totals[gender] + 1;
                                        htsscreen = 1;
                                        params.Totalscreened = 1;
                                        var myresult = "";
                                        let optn = data[i].observations.find(x => x.concept_id == "7363").value_text;
                                        let sti_results = optn.toLowerCase();
                                        if (sti_results == "nn") {
                                            myresult = "negative";
                                        }
                                        ...
                                        if (sti_results != "inconclusive") {
                                            this.DataObjects.find(x => x.module == "HTS")[myresult] = totals[myresult] + 1;
                                        }
                                    }
                                }
                            }
                    ...
                    if (data[i].encounter_type == 58) {
                        if (covidscreen == 0) {
                            for (var x = 0; x < data[i].observations.length; x++) {
                                let conceptid = data[i].observations[x].concept_id;
                                var totals = this.DataObjects.find(x => x.module == "COVID");
                                if (conceptid == 1837) {
                                    let screened = data[i].observations[x].value_datetime.split('T')[0];
                                    if (screened != null) {
                                        this.DataObjects.find(x => x.module == "COVID")[gender] = totals[gender] + 1;
                                        covidscreen = 1;
                                        params.Totalscreened = 1;
                                        let optn = data[i].observations.find(x => x.concept_id == "2169").value_text;
                                        let sti_results = optn.toLowerCase();
                                        if (sti_results != null) {
                                            this.DataObjects.find(x => x.module == "COVID")[sti_results] = totals[sti_results] + 1;
                                        }
                                    }
                                }
                            }
                    ...
                    if (data[i].encounter_type == 154) {
                        if (cxcascreen == 0) {
                            for (var x = 0; x < data[i].observations.length; x++) {
                                let conceptid = data[i].observations[x].concept_id;
                                var totals = this.DataObjects.find(x => x.module == "CXCA");
                                if (conceptid == 8387) {
                                    let screened = data[i].observations[x].value_text;
                                    if (screened == "Yes") {
                                        this.DataObjects.find(x => x.module == "CXCA")[gender] = totals[gender] + 1;
                                        cxcascreen = 1;
                                        params.Totalscreened = 1;
                                        let optn = data[i].observations.find(x => x.concept_id == "6538").value_text;
                                        let sti_results = optn.toLowerCase();
                                        if (sti_results != null) {
                                            this.DataObjects.find(x => x.module == "CXCA")[sti_results] = totals[sti_results] + 1;
                                        }
                                    }
                                }
                            }
    ...
    populateData(data) {
        let ttals = data.Totalscreened;
        let gender = data.gender;
        let totl = this.ScreenedTotal.find(x => x.Population_Type == gender)["Value"];
        this.ScreenedTotal.find(x => x.Population_Type == gender)["Value"] = totl + ttals;
    }
    loadBidChartData() {
        var firstDay;
        var lastDay;
    ...
    GenerateData() {
        ...
        let start = this.myReports.get('start').value;
        let end = this.myReports.get('end').value;
        let startsmonth = this.datepipe.transform(start, "MMM-y");
        let startsyear = this.datepipe.transform(start, "y");
        let endsmonth = this.datepipe.transform(end, "MMM-y");
        let endsyear = this.datepipe.transform(end, "y");
        let statdate = this.datepipe.transform(start, "y-MM-d");
        let enddate = this.datepipe.transform(end, "y-MM-d");
